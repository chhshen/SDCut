...
% ON REMET LAMBDA A 0
lambda = param.optim.lambda_init;
...
while stop_outer==0,
    ...
    if rho > rho_prime,
        if param.optim.mod==1 && lambda<10e10
            lambda=param.optim.beta*lambda;
        elseif param.optim.mod==2 && old_h~=0;
            if new_h/old_h>param.optim.gamma
                lambda=param.optim.beta*lambda;
            end
        elseif param.optim.mod==3 && ~isempty(old_h_V)
            lambda=( param.optim.beta.*((new_h_V-old_h_V.*param.optim.gamma)>0)+1.*((new_h_V-old_h_V.*param.optim.gamma)<=0) ).*lambda;
        end
        Delta = Delta0;
    end
